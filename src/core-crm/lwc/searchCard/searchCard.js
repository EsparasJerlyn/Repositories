...
export default class SearchCard extends LightningElement {
   ...
   @track categoryName = '';
   @track hasCategory = false;
   
    /**
     * An event fired when the user clicked on the action button (add to cart)
     *
     * Properties:
     *   - Bubbles: true
     *   - Composed: true
  ...
  get resolvedEffectiveAccountId() {
    ...
    get showPriceIfStudy(){
        return window.location.href.indexOf(STUDY_STORE) > -1 ? true : false;
    }

    handleDisplayProductDesc(event) {
        let productDesc = event.detail;
        if(productDesc != '') {
            this.productDescription = htmlDecode(event.detail);
        }
    }

    handleDisplayCategoryName(event) {
        let catName = event.detail;
        if(catName != '') {
