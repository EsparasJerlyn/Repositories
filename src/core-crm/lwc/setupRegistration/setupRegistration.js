...
import ANSWER_OBJECT from '@salesforce/schema/Answer__c';
...
export default class SetupRegistration extends LightningElement {
    ...
    get questionnaireOptions(){
        ...
        if(this.questionnaireTypeOptions.data){
            let _existingTypes = this.registrationData.map(navItem => {return navItem.type});
            _options = this.questionnaireTypeOptions.data.values.filter(item => !_existingTypes.includes(item.value));
        }
        return _options;
    }
    ...
    get showColumnHeaders(){
        return this.selectedQuestionnaire && this.selectedQuestionnaire.relatedQuestions.length > 0;
    }

    //returns api name of record to be created
    get createRecordObject(){
        return this.createRecordLabel == STR_QUESTION ? this.questionApiName : QUESTIONNAIRE_OBJECT.objectApiName;
    }
    ...
    get sequenceInvalid(){
        let sequences = this.selectedQuestionnaire.relatedQuestions.filter( (row) => 
            row.sequence !== '' && row.sequence > 0 &&
            row.sequence <= this.selectedQuestionnaire.relatedQuestions.length).map(row => {
            return row.sequence
        });
        return [...new Set(sequences)].length !== this.selectedQuestionnaire.relatedQuestions.length;
    }
    ...
    @wire(getPicklistValues, { recordTypeId: '$questionnaireObjectInfo.data.defaultRecordTypeId', fieldApiName: QUESTIONNAIRE_TYPE })
    questionnaireTypeOptions;

    //calls apex method and stores question, questionnaire, and answer records
    registrationResult;
    ...
    handleGetRegistrationData(result){
        ...
        if(result.data){
            ...
            this.registrationData = this.registrationResult.data.questionnaireList.map(data =>{
                let questionnaire = {};
                ...
                questionnaire.relatedQuestions = this.allQuestions.length > 0 ? this.formatRelatedQuestions(data.Id) : [];
            ...
            if(this.registrationData.length > 0){
                if(this.initialLoad){
                    ...
                    this.setSelectedQuestionnaire(this.registrationData.find(data => data.id == this.selectedQuestionnaire.id));
                }
    ...
    formatRelatedQuestions(questionnaireId){
        ...
        this.allQuestions.forEach(question => {
            let relatedAnswer = question.Answers__r ? question.Answers__r.find(answer => answer.Questionnaire__c == questionnaireId) : null;
            if(relatedAnswer){
                let _question = {};
                ...
                _question.editable = question.Answers__r.find(answer => answer.Name < relatedAnswer.Name) ? false : true;
    ...
    getAvailableQuestions(){
        this.availableQuestions = this.allQuestions.filter(question => 
            !this.selectedQuestionnaire.relatedQuestions.map(rq => {return rq.id})
            .includes(question.Id)
        );
        this.availableQuestions = this.availableQuestions.sort((a, b) => a.Label__c.localeCompare(b.Label__c));
    ...
    setActiveNav(){
        this.template.querySelector(`[data-id="${this.selectedQuestionnaire.id}"]`).classList.add('slds-is-active');
    }

    //removes active class from previously selected questionnaire
    removeActiveNav(){
        this.template.querySelector(`[data-id="${this.selectedQuestionnaire.id}"]`).classList.remove('slds-is-active');
    }
    ...
    handleSubmitRecord(event){
        ...
        if(this.createRecordLabel == STR_QUESTIONNAIRE){
            ...
            fields[QUESTIONNAIRE_TYPE.fieldApiName] = this.selectedQuestionnaireOption; 
    ...
    handleSuccessRecord(event){
        ...
        .then(() => {
            if(this.createRecordLabel == STR_QUESTIONNAIRE){    
                ...
                this.setSelectedQuestionnaire(this.registrationData[this.registrationData.length-1]);
                this.setActiveNav();
                this.generateToast('Success!',STR_QUESTIONNAIRE + ' created.','success');
    ...
    handleNavSelect(event) {
        this.removeActiveNav();
        this.setSelectedQuestionnaire(this.registrationData.find(item => item.name == event.target.dataset.name));
    ...
    handleQuestionSearch(event){
        let filterString = event.target.value;
         if(filterString){
             this.availableQuestionsToDisplay = this.availableQuestions.filter(question =>
    ...
    handleRemoveQuestion(event){
        this.isLoading = true;
        let seq = this.selectedQuestionnaire.relatedQuestions.find(rq => rq.answerId == event.target.dataset.name).sequence;
        deleteRecord(event.target.dataset.name)
        .then(() => {
            let answersToUpdate = this.selectedQuestionnaire.relatedQuestions.filter(draft =>
                draft.sequence > seq
    ...
    handleSequenceChange(event){
        ...
        this.selectedQuestionnaire.relatedQuestions = this.selectedQuestionnaire.relatedQuestions.map(row=>({
            ...row,sequence:event.target.name === row.answerId ? parseInt(event.target.value) : row.sequence
        }));
     }
    ...
    handleSaveSequence(){
        let answersToUpdate = this.selectedQuestionnaire.relatedQuestions.filter(draft =>
            this.draftRelatedQuestions.find(answer => answer.answerId == draft.answerId).sequence !== draft.sequence
